...
function post_redirect($url)
{
    ob_start();
    header('Location: ' . $url);
    // header('Location: https://md-taha-ahmed.000webhostapp.com/pharmeasy/doctor/' . $url);
    ob_end_flush();
    die();
}
function get_redirect($url)
{
    ...
    // echo "<script>
    // window.location.href = 'https://md-taha-ahmed.000webhostapp.com/pharmeasy/doctor/" . $url . "';
    // </script>";
}
...
function message()
{
    if ($_SESSION['message'] == "loginErr") {
        ...
        unset($_SESSION['message']);
    } elseif ($_SESSION['message'] == "noResultDoctor") {
        echo "   <div class='alert alert-danger' role='alert'>
        There is no doctor with this email !!!
      </div>";
        unset($_SESSION['message']);
...
function login()
{
    if (isset($_POST['login'])) {
        ...
        $password = trim(strtolower($_POST['doctorPassword']));
        $query = "SELECT  doctor_email , doctor_id , doctor_password FROM doctor WHERE doctor_email= '$doctorEmail' ";
        $data = query($query);
        if ($data == 0) {
            $_SESSION['message'] = "loginErr";
            post_redirect("login.php");
        } elseif ($password == $data[0]['doctor_password'] and  $doctorEmail == $data[0]['doctor_email']) {
            $_SESSION['doctor_id'] = $data[0]['doctor_id'];
...
{
    $query = "SELECT doctor_id ,doctor_fname ,doctor_lname ,doctor_email  FROM doctor";
    $data = query($query);
    return $data;
}
function get_doctor($id)
{
    $query = "SELECT doctor_id ,doctor_fname ,doctor_lname ,doctor_email  FROM doctor WHERE doctor_id=$id";
    $data = query($query);
    return $data;
}
function edit_doctor($id)
{
    if (isset($_POST['doctor_update'])) {
        $fname = trim($_POST['doctor_fname']);
        $lname = trim($_POST['doctor_lname']);
        $email = trim(strtolower($_POST['doctor_email']));
        $password = trim($_POST['doctor_password']);
        $check = check_email_doctor($email);
        if ($check == 0) {
            $query = "UPDATE doctor SET doctor_email='$email' ,doctor_fname='$fname' ,doctor_lname='$lname' ,doctor_password='$password'  WHERE doctor_id= '$id'";
            single_query($query);
    ...
    } elseif (isset($_POST['doctor_cancel'])) {
        get_redirect("doctor.php");
    }
}
function check_email_doctor($email)
{
    $query = "SELECT doctor_email FROM doctor WHERE doctor_email='$email'";
    $data = query($query);
    if ($data) {
        return $data;
    } else {
...
function add_doctor()
{
    if (isset($_POST['add_doctor'])) {
        ...
        $check = check_email_doctor($email);
        if ($check == 0) {
            $query = "INSERT INTO doctor (doctor_fname, doctor_lname, doctor_email, doctor_password) 
...
function search_doctor()
{
    if (isset($_GET['search_doctor'])) {
        $email = trim(strtolower($_GET['search_doctor_email']));
        if (empty($email)) {
            return;
        }
        $query = "SELECT doctor_id ,doctor_fname ,doctor_lname ,doctor_email FROM doctor WHERE doctor_email='$email'";
        $data = query($query);
        if ($data) {
            return $data;
        } else {
            $_SESSION['message'] = "noResultdoctor";
            return;
        }
    }
}
function check_doctor($id)
{
    $query = "SELECT doctor_id FROM doctor where doctor_id='$id'";
    $row = query($query);
    if (empty($row)) {
        return 0;
    } else {
